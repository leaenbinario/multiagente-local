...
BASE_SHARED = Path(__file__).resolve().parent.parent / "shared-data"

CARPETA_DOCS = BASE_SHARED / "documentos_contables"
CARPETA_PROCESADOS = BASE_SHARED / "procesados_contables"
...
def leer_pdf(path: Path) -> str:
    ...
    2) Si no obtiene nada útil, intenta OCR.
    """
    try:
        textos = []
...
# ==============================
# 4) Inicializar Chroma + embeddings (CONTADOR)
# ==============================
...
def ingestar_carpeta(carpeta_docs: Path, carpeta_procesados: Path):
    ...
    for path in archivos:
        ...
        print(f"  Ingestados {total_chunks} chunks (nuevos y/o ya existentes) en colección {COLLECTION_NAME}.")
...
if __name__ == "__main__":
    ...
    if not CARPETA_DOCS.exists():
        print("La carpeta documentos_contables no existe. Créala y coloca allí .txt, .pdf, .eml, .docx, .odt, .odp, .pptx.")
    else:
